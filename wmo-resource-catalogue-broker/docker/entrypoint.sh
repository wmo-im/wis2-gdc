...
USERNAME=`echo $WMO_RESOURCE_CATALOGUE_BROKER_URL |awk -F/ '{print $3}' | awk -F@ '{print $1}' | awk -F: '{print $1}'`
PASSWORD=`echo $WMO_RESOURCE_CATALOGUE_BROKER_URL |awk -F/ '{print $3}' | awk -F@ '{print $1}' | awk -F: '{print $2}'`
...
if [ ! -e "/mosquitto/config/password.txt" ]; then
    echo "Adding wmo-resource-catalogue users to mosquitto password file"
    ...
    echo "Mosquitto password file already exists. Skipping wmo-resource-catalogue user addition."
fi
